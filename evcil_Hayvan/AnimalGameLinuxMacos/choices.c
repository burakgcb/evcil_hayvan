...
void print_animal_names()
{
    ...
    my_printf("Islem yapabileceginiz hayvanlarin isimleri :\n");
    while(tmp = my_next_line(fd))
    {
        my_printf("%s\n",my_split(tmp,',')[0]);
    }
...
void uptade_name(struct Animal **new, int fd)
{
    char *buffer = NULL;
    buffer = malloc(1000);
    my_printf("Yeni bir isim giriniz.\n");
    if (readline(0, &buffer, 260))
    {
        (new[0])->name = my_strdup(buffer);
    }
    free(buffer);
    my_putstr_fd("Ismi degistirildi.",fd);
}

void foofd_animal_(struct Animal **ref,int fd)
{
    my_printf("%s, isimli hayvan beslendi.", (ref[0])->name);
     my_putstr_fd("Yemek verildi.",fd);
    (ref[0])->energy += 15;
    (ref[0])->happiness += 10;
}
void water_animal_(struct Animal **ref, int fd)
{
    my_printf("%s, isimli hayvana su verildi.", (ref[0])->name);
     my_putstr_fd("Su içirildi.",fd);
    (ref[0])->energy += 5;
    (ref[0])->happiness += 5;
}
void play_animal_(struct Animal **ref, int fd)
{
    my_printf("%s, isimli hayvan oyun oynatildi.", (ref[0])->name);
     my_putstr_fd("Oyun oynatildi.",fd);
    (ref[0])->energy -= 15;
    (ref[0])->happiness += 15;
...
{
    my_printf("%s, isimli hayvan temizlendi.", (ref[0])->name);
     my_putstr_fd("Temizlendi.",fd);
...
{
    my_printf("%s, isimli hayvan sevildi.", (ref[0])->name);
     my_putstr_fd("Sevildi.",fd);
    (ref[0])->happiness += 5;
}
void sleep_animal_(struct Animal **ref, int fd)
{
    my_printf("%s, isimli hayvan uyutuldu.", (ref[0])->name);
    my_putstr_fd("Uyutuldu.",fd);
    (ref[0])->energy += 5;
}
void price_animal_(struct Animal **ref, int fd)
{
    my_printf("%s, isimli hayvan ödüllendirildi.", (ref[0])->name);
    my_putstr_fd("Odullendirildi.",fd);
    (ref[0])->happiness += 10;
}
...
void detail_animal(struct Animal **cpy, int fd)
{
    ...
    {
        (*cpy)->emotion = my_strdup("mutlu");
        (*cpy)->emotion_txt = my_strdup(":)");
    }
    else if((*cpy)->energy < 20 && (*cpy)->happiness < 20)
    {
        (*cpy)->emotion = my_strdup("uzgun");
        (*cpy)->emotion_txt = my_strdup(":(");
    }
    else if((*cpy)->energy < 10 && (*cpy)->happiness < 10)
    {
        (*cpy)->emotion = my_strdup("agliyor");
        (*cpy)->emotion_txt = my_strdup(":')");
    }
    else if((*cpy)->energy < 5)
    {
        (*cpy)->emotion = my_strdup("uyuyor");
        (*cpy)->emotion_txt = my_strdup(":zz");
    }
    else if((*cpy)->energy < 20)
    {
        (*cpy)->emotion = my_strdup("ac");
        (*cpy)->emotion_txt = my_strdup(":-o");
    }
    else if((*cpy)->energy > 40 || (*cpy)->happiness < 30)
    {
        (*cpy)->emotion = my_strdup("Oyun Istiyor");
        (*cpy)->emotion_txt = my_strdup("<:o)");
    }
    my_printf("%s isimli hayvan kendini %s hissediyor. Enerji seviyesi : %d mutluluk seviyesi: %d \n", (*cpy)->name ,(*cpy)->emotion, (*cpy)->energy, (*cpy)->happiness);
    multi_putstr(cpy,fd);
    ...
    switch (my_atoi(choice)) {
        ...
        case 6:
            if (animal->happiness > 5)
                ...
                my_printf("%s isimli hayvan mutlu degil. Onu mutlu edecek seyler yapin.\n", animal->name);
            break;
        case 7:
        ...
        case 10:
            ...
            return 0;
        default:
            my_printf("Yanlis tuslama yaptiniz.\n");
    }
